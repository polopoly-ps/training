...
public class PollElementController extends RenderControllerExtended {
    ...
    public static final String AJAX_PAGE = "example.AjaxPageLayout.ot";
    private static final Map<String, String> PARAMS = Collections.singletonMap("ot", AJAX_PAGE);
            ...
            ControllerContext context)
    {
        ...
        try {
            ...
            SinglePollPolicy singlePollPolicy =
                    (SinglePollPolicy) singlePollModel.getAttribute("_data");

            // View is set to 'form-view' as default
            SinglePollElementBean bean = new SinglePollElementBean();
            bean.setRegisterPollId(singlePollPolicy.getPollId());
            ...
            populateAjaxAttributes(context, request, localModel);
        } catch (CMException e) {
            LOG.log(Level.WARNING, "Problem showing poll view.", e);
        }
    }
    
    protected void populateAjaxAttributes(ControllerContext context, RenderRequest request, ModelWrite model) throws CMException
    {
        boolean isAjax = AJAX_PAGE.equals(request.getParameter("ot"));
        model.setAttribute("isAjax", isAjax);
        HttpServletRequest httpServletRequest = (HttpServletRequest) request;
        URLBuilder urlBuilder = RequestPreparator.getURLBuilder(httpServletRequest);
        ContentPathCreator path = RequestPreparator.getPathCreator(httpServletRequest);
        PolicyCMServer policyCMServer = _policyCMServerProvider.getPolicyCMServer(context);
        String urlToSelf = urlBuilder.createUrl(path.createPath(context.getContentId(), policyCMServer), PARAMS, httpServletRequest);
        model.setAttribute("urlToSelf", urlToSelf);
    }
